#!/usr/bin/env python
"""
Worker script for parallel generation of 6-point spinor-helicity oracle data.
Uses TermSpecificActionSpace6pt for 6-point amplitudes.

Key differences from 5-point (generate_oracle_worker_5pt.py):
- 30 brackets (15 ab + 15 sb) instead of 20
- 32 identity types per bracket (6 Schouten + 10 Momentum + 16 Momentum_sq)
- Total action space: 30 * 31 * 32 = 29,760 actions
"""
...
from action_space_6pt import (
    TermSpecificActionSpace6pt,
...
def expressions_equal(expr1: sp.Expr, expr2: sp.Expr) -> bool:
    ...
    Uses sp.together() which is faster than cancel+expand for rational functions.
    """
    if expr1 == expr2:
        return True
    try:
        diff = sp.together(expr1 - expr2)
        if diff == 0:
            return True
        # Fallback: check if numerator is zero after expansion
        num, _ = sp.fraction(diff)
        if num == 0 or sp.expand(num) == 0:
            return True
    except:
        pass
    return False
...
def apply_identity_full(sp_expr: sp.Expr, bracket: sp.Function, identity: str,
    ...
    Uses sp.together() instead of sp.cancel() for speed - results are
    mathematically equivalent and will compare correctly.
    """
    ...
    if numerator_only:
        ...
        return sp.together(num / denom)
    else:
        return sp.together(sp_expr.subs(bracket, identity_result))
...
def compute_unscrambling_action_6pt(
    ...
    """Compute the unscrambling action using 6pt term-specific action space."""
    ...
    for spec in candidate_specs:
        ...
        try:
            ...
            # term_slot=1-30: specific term (term_slot-1)
...
def generate_sample_with_oracle_6pt(
    n_point: int,
    max_scrambles: int,
    min_scrambles: int,
    max_scale: int,
    ...
    """Generate a sample with oracle trajectory using 6pt term-specific action space."""
    try:
        ...
        for i in range(len(reversed_states) - 1):
            ...
            action = compute_unscrambling_action_6pt(
...
def main():
    parser = argparse.ArgumentParser(description='Worker for 6pt oracle data generation')
    ...
    parser.add_argument('--max_scale', type=int, default=2, help='Max scale for amplitude generation')
    parser.add_argument('--max_terms_gen', type=int, default=3, help='Max terms for generation')
    parser.add_argument('--max_terms', type=int, default=30, help='Max terms for action space')
    parser.add_argument('--max_brackets', type=int, default=30, help='Max brackets (30 for 6-point)')
    parser.add_argument('--l_scale', type=float, default=0.75, help='Scale factor for generation')
    parser.add_argument('--all_momenta', action='store_true', default=False,
                        help='Use all momenta in generation')
    ...
    assert args.n_point == 6, "This script is specifically for 6-point amplitudes"
    ...
    while len(samples) < my_samples and attempts < max_attempts:
        attempts += 1
        sample = generate_sample_with_oracle_6pt(
            n_point=args.n_point,
    ...
    output_path = os.path.join(args.output_dir, f'oracle_6pt_{args.worker_id:04d}.pkl')
