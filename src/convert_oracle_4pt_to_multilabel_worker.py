"""
Worker script for parallel conversion of v10 oracle data to multi-label transitions.
Uses TermSpecificActionSpace (action_space_4pt) with term_slot semantics.
...
def convert_chunk(
    ...
    for sample in oracle_data:
        ...
        for step in sample['trajectory']:
            ...
            # Build action mask
            action_mask = build_action_mask_v3(state_expr, action_space)
...
def main():
    parser = argparse.ArgumentParser(description='Worker for v10 oracle to transitions conversion')
    ...
    parser.add_argument('--max_brackets', type=int, default=12,
                        help='Max brackets for action space (12 for 4-point)')
    args = parser.parse_args()

    print(f"Worker {args.worker_id}/{args.num_workers} starting...", flush=True)
    print(f"Action space: {12} brackets x {args.max_terms_action + 1} term_slots x 11 identities = {12 * (args.max_terms_action + 1) * 11} actions", flush=True)
