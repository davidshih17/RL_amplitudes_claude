...
Version 4 - 6pt: Uses term-specific action space for 6-point amplitudes.

Key features:
- Uses 29760 action space (30 brackets * 31 term_slots * 32 identities)
- 32 identities per bracket: 6 Schouten + 10 Momentum + 16 Momentum_sq
- term_slot=0: apply to ENTIRE expression, term_slot=1-30: single term
- Retry on timeout with next best action

Adapted from eval_sft_5pt.py for 6-point amplitudes.
"""
        ...
        try:
            ...
            try:
                ...
                if term_slot == 0:
                    ...
                    # Term-specific application (term_slot=1-30 -> term_idx=0-29)
...
def init_worker(n_point, max_terms=30):
...
def build_action_mask_6pt(expr, action_space: TermSpecificActionSpace6pt, numerator_only: bool = True):
...
def evaluate_sample(
    ...
    for step in range(max_steps):
        ...
        # Build action mask
        t1 = time.time()
        action_mask = build_action_mask_6pt(current_expr, action_space)
        t_mask = time.time() - t1
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate 6pt SFT model with term-specific action space')
    ...
    if args.random_policy:
        print("Evaluating RANDOM POLICY BASELINE - 6pt Term-Specific Action Space")
    else:
        print("Evaluating 6pt SFT Model (v4) - Term-Specific Action Space")
    print("=" * 70)
    ...
    if args.random_policy:
        ...
        # Compute dimensions
        bracket_feature_dim = 2 + 2 * args.n_point  # 14 for 6pt
        term_feature_dim = 2 + args.max_brackets_per_term * bracket_feature_dim + 1  # 283
