"""
Action space for 6-point amplitudes with term-specific identity application.

Key differences from 5-point (action_space_5pt.py):
- 30 brackets: C(6,2) * 2 = 15 pairs * 2 types (ab/sb) = 30
- 6 Schouten identity types per bracket (C(4,2) = 6 pairs from 4 other momenta)
- 10 Momentum identity types per bracket (2 pout1 * 5 pout2)
- 16 Momentum_sq identity types per bracket (2^4 = 16 subsets of 4 other momenta)
- Total per bracket: 6 + 10 + 16 = 32 identity types
- 31 term slots: 1 "whole expression" + 30 individual terms
- Total actions: 30 * 31 * 32 = 29,760 actions

Action encoding:
    action = bracket_idx * (n_term_slots * identities_per_bracket) + term_slot * identities_per_bracket + identity_type_idx

Where:
- bracket_idx: 0-29 (which of the 30 possible brackets)
- term_slot: 0-30 (0 = entire expression, 1-30 = specific term index 0-29)
- identity_type_idx: 0-31 (which identity variant)
"""
...
class ActionSpec(NamedTuple):
    ...
    bracket_idx: int  # Index in canonical bracket ordering (0-29 for 6-point)
    term_slot: int    # 0 = whole expression, 1-30 = specific term (term_slot-1)
    identity: str     # 'schouten', 'momentum', or 'momentum_sq'
    identity_type_idx: int  # Index within identity types for this bracket (0-31)
    ...
    def term_idx(self) -> Optional[int]:
        """Get the actual term index (0-29) for single-term actions, or None for whole expression."""
        return (self.term_slot - 1) if self.term_slot > 0 else None


class TermSpecificActionSpace6pt:
    """
    Action space with term-specific identity application for 6-point amplitudes.

    For 6-point amplitudes:
    - 30 canonical brackets (sorted deterministically)
    - 31 term slots: 1 "whole expression" (slot 0) + 30 individual terms (slots 1-30)
    - 32 identity types per bracket (6 Schouten + 10 Momentum + 16 Momentum_sq)
    - Total: 30 * 31 * 32 = 29,760 actions

    Two action types:
    1. term_slot=0: Apply identity to ALL occurrences in ENTIRE expression
    2. term_slot=1-30: Apply identity to ONE instance in term (term_slot-1)
    """

    def __init__(self, n_point: int = 6, max_terms: int = 30, max_brackets: int = 30):
        """
        Args:
            n_point: Number of external momenta (default 6)
            max_terms: Maximum number of terms (default 30)
            max_brackets: Maximum number of brackets (default 30 for 6-point)
        """
        assert n_point == 6, "This action space is specifically for 6-point amplitudes"
        ...
        # For 6-point: 6 + 10 + 16 = 32 identity types per bracket
    ...
    def _compute_identities_per_bracket(self) -> int:
        ...
        # For n_point=6: 4 other momenta besides bracket args
        n_other = self.n_point - 2  # = 4 for 6-point

        # Schouten: C(n_other, 2) distinct pairs (swapping pk/pl gives same result)
        n_schouten = n_other * (n_other - 1) // 2  # C(4,2) = 6

        # Momentum: 2 pout1 choices * (1 + n_other) pout2 choices
        n_momentum = 2 * (1 + n_other)  # 2 * 5 = 10

        # Momentum_sq: all subsets of other momenta
        n_momentum_sq = 2 ** n_other  # 2^4 = 16

        return n_schouten + n_momentum + n_momentum_sq  # 6 + 10 + 16 = 32
    ...
    def get_bracket_idx(self, bracket: sp.Function) -> int:
        ...
        Returns:
            Canonical index (0-29 for 6-point)
        """
    ...
    def get_bracket_from_idx(self, bracket_idx: int) -> Tuple[str, int, int]:
        ...
        Args:
            bracket_idx: Canonical index (0-29 for 6-point)
    ...
    def get_identity_types_for_bracket(self, bracket_idx: int) -> List[Dict[str, Any]]:
        ...
        Returns:
            ...
            - 'local_idx': index within this bracket's identities (0-31)
        """
    ...
    def encode_action(
        ...
        Args:
            bracket_idx: Canonical bracket index (0-29)
            term_slot: 0 = whole expression, 1-30 = specific term (term_slot-1)
            identity_type_idx: Identity type index (0-31)

        Returns:
            Action index (0-29759 for default config)
        """
    ...
    def decode_action(self, action: int) -> Tuple[int, int, int]:
        ...
        Returns:
            ...
            where term_slot: 0 = whole expression, 1-30 = specific term
        """
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("Testing TermSpecificActionSpace6pt")
    print("=" * 70)

    # Create action space
    action_space = TermSpecificActionSpace6pt(n_point=6, max_terms=30, max_brackets=30)
    ...
    print("\n" + "=" * 70)
    print("Comparison: 5pt vs 6pt action spaces")
    print("=" * 70)

    from action_space_5pt import TermSpecificActionSpace5pt
    as5 = TermSpecificActionSpace5pt(n_point=5, max_terms=25, max_brackets=20)

    print(f"\n5-point:")
    print(f"  Brackets: {len(as5.canonical_brackets)}")
    print(f"  Identities per bracket: {as5.identities_per_bracket}")
    print(f"  Total actions: {as5.n_actions}")
    ...
    test_cases = [
        ...
        (0, 0, 5),   # First bracket, WHOLE EXPRESSION, 6th identity (Schouten 5,6)
        (0, 1, 0),   # First bracket, term 0, first identity
        (29, 30, 31), # Last bracket, last term, last identity
    ]
