...
Version 4 - 5pt: Uses term-specific action space for 5-point amplitudes.

Key features:
- Uses 9880 action space (20 brackets * 26 term_slots * 19 identities)
- 19 identities per bracket: 3 Schouten + 8 Momentum + 8 Momentum_sq
- term_slot=0: apply to ENTIRE expression, term_slot=1-25: single term
- Retry on timeout with next best action

Adapted from eval_sft_4pt.py for 5-point amplitudes.
"""
        ...
        try:
            ...
            try:
                ...
                if term_slot == 0:
                    ...
                    # Term-specific application (term_slot=1-25 -> term_idx=0-24)
...
def init_worker(n_point, max_terms=25):
...
def apply_action_with_timeout(expr, bracket_type, bracket_args, identity, params, term_slot, n_point, timeout_sec):
    ...
    Args:
        term_slot: 0 = whole expression, 1-25 = specific term (term_slot-1)
    """
    global _worker_process, _request_queue, _response_queue
...
from action_space_4pt import get_unique_brackets, get_term_bracket_powers
from environment.bracket_env import ab, sb
...
def build_action_mask_5pt(expr, action_space: TermSpecificActionSpace5pt, numerator_only: bool = True):
    """Build action mask for 5pt term-specific action space.

    term_slot=0: whole expression (valid if bracket exists anywhere)
    term_slot=1-25: specific term (valid if bracket exists in that term)
    """
    ...
    for bracket in brackets:
        ...
        # term_slot=1-25: specific term - valid if bracket in that term
...
def evaluate_sample(
    ...
    for step in range(max_steps):
        ...
        # Build action mask
        t1 = time.time()
        action_mask = build_action_mask_5pt(current_expr, action_space)
        t_mask = time.time() - t1
...
def evaluate_sample_backtrack(
    ...
    max_term_increase: int = 5,
    random_policy: bool = False,
    proper_stop: bool = False,
    checkpoint_by_terms: bool = False,
    reject_term_increase: int = None,
    rti_schedule: list = None,
) -> Dict[str, Any]:
    """Evaluate with backtracking on dead ends and max_steps failures.

    Args:
        reject_term_increase: If set, reject actions that increase terms by more than this amount.
        rti_schedule: If set, list of RTI values to escalate through when backtracks are
            exhausted (e.g., [1, 3, 5]). Overrides reject_term_increase.
    ...
    # RTI escalation setup
    if rti_schedule is not None and len(rti_schedule) > 0:
        rti_idx = 0
        reject_term_increase = rti_schedule[0]
    else:
        rti_schedule = None
        rti_idx = None

    start_brackets = count_brackets(initial_expr)
    target_brackets = count_brackets(target_expr) if target_expr is not None else 0
    min_brackets = start_brackets
    start_terms = count_terms(initial_expr)
    target_terms = count_terms(target_expr) if target_expr is not None else 0
    ...
    if verbose:
        ...
        if target_expr is not None:
            ...
            print(f"    target expr  = None (source-relative mode)", flush=True)
    ...
    while step < max_steps:
        ...
        if not action_succeeded:
            ...
            # Try escalating RTI before giving up
            if rti_schedule is not None and rti_idx < len(rti_schedule) - 1:
                rti_idx += 1
                reject_term_increase = rti_schedule[rti_idx]
                backtrack_count = 0  # reset backtrack budget
                if verbose:
                    print(f"    Step {step}: no valid action at RTI={rti_schedule[rti_idx-1]}, "
                          f"escalating to RTI={reject_term_increase}", flush=True)
                continue
    ...
    # Post-loop RTI escalation: if backtracks exhausted without success, try higher RTI
    # and re-run the post-loop backtracking with the new RTI
    while not reached_target and not solved_early and rti_schedule is not None and rti_idx < len(rti_schedule) - 1:
        rti_idx += 1
        reject_term_increase = rti_schedule[rti_idx]
        backtrack_count = 0  # reset backtrack budget
        if verbose:
            print(f"    Post-loop: backtracks exhausted at RTI={rti_schedule[rti_idx-1]}, "
                  f"escalating to RTI={reject_term_increase}", flush=True)

        # Re-run backtracking with higher RTI
        while backtrack_count < max_backtracks and len(checkpoints) > 1:
            backtrack_start = time.time()

            best_idx = 0
            if checkpoint_by_terms:
                best_metric = checkpoints[0][2]
                for i, cp in enumerate(checkpoints):
                    if cp[2] < best_metric:
                        best_metric = cp[2]
                        best_idx = i
            else:
                best_metric = checkpoints[0][1]
                for i, cp in enumerate(checkpoints):
                    if cp[1] < best_metric:
                        best_metric = cp[1]
                        best_idx = i
            ...
            if action_that_left is not None:
                saved_blocked.add(action_that_left)
                checkpoints[best_idx][5] = None

            if verbose:
                print(f"    BACKTRACK #{backtrack_count+1} (RTI={reject_term_increase}) to checkpoint with "
                      f"{saved_brackets} brackets, {saved_terms} terms (blocking action {action_that_left})", flush=True)
            ...
            for extra_step in range(extra_steps):
                ...
                if proper_stop:
                    if current_brackets <= target_brackets and current_terms <= target_terms:
                        if verbose:
                            print(f"    Extra step {extra_step}: reached proper target", flush=True)
                        reached_target = True
                        break
                else:
                    if current_brackets <= target_brackets:
                ...
                for retry_idx, action in enumerate(sorted_actions[:max_retries + 20]):
                    ...
                    if result is None or result == current_expr:
                        continue
                    ...
                    if verbose:
                        ...
                        retry_str = f" (retry={retry_idx})" if retry_idx > 0 else ""
                        print(f"    Extra step {extra_step}: {current_brackets}->{new_brackets} terms={new_terms} "
                              f"via {spec.identity} {bracket_type}({ii},{jj}) {ts_str}{retry_str} "
                              f"[bt#{backtrack_count}, RTI={reject_term_increase}]", flush=True)
        ...
        for sample_idx, sample in enumerate(tqdm(test_samples, desc=desc)):
            ...
            if n_rollouts > 1:
                ...
                for rollout_idx in range(n_rollouts):
                    ...
                    rollout_verbose = verbose and (rollout_idx == 0)
                    ...
                    if use_backtrack:
                        r = evaluate_sample_backtrack(
                            ...
                            max_term_increase=max_term_increase,
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate 5pt SFT model with term-specific action space')
    ...
    parser.add_argument('--max_terms', type=int, default=25)
    parser.add_argument('--max_terms_action', type=int, default=25)
    parser.add_argument('--max_brackets_per_term', type=int, default=8)
    parser.add_argument('--max_brackets', type=int, default=20)
    ...
    parser.add_argument('--max_term_increase', type=int, default=5, help='Max term increase before backtracking')
    ...
    # Reject term increase
    parser.add_argument('--reject_term_increase', type=int, default=None,
                        help='Reject actions that increase terms by more than this amount')
    ...
    if args.random_policy:
        print("Evaluating RANDOM POLICY BASELINE - 5pt Term-Specific Action Space")
    else:
        print("Evaluating 5pt SFT Model (v4) - Term-Specific Action Space")
    print("=" * 70)

    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(f"Using device: {device}")

    # Create 5pt action space
    action_space = TermSpecificActionSpace5pt(
    ...
    if args.random_policy:
        ...
        # Compute dimensions
        bracket_feature_dim = 2 + 2 * args.n_point  # 12 for 5pt
        term_feature_dim = 2 + args.max_brackets_per_term * bracket_feature_dim + 1  # 99
    ...
    results = evaluate_on_dataset(
        ...
        max_term_increase=args.max_term_increase,
