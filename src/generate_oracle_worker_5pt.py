#!/usr/bin/env python
"""
Worker script for parallel generation of 5-point spinor-helicity oracle data.
Uses TermSpecificActionSpace5pt for 5-point amplitudes.

Key differences from 4-point (generate_oracle_worker_4pt.py):
- 20 brackets (10 ab + 10 sb) instead of 12
- 19 identity types per bracket (3 Schouten + 8 Momentum + 8 Momentum_sq)
- Total action space: 20 * 11 * 19 = 4180 actions
"""
...
from action_space_5pt import (
    TermSpecificActionSpace5pt,
...
def compute_unscrambling_action_5pt(
    ...
    """Compute the unscrambling action using 5pt term-specific action space."""
...
def generate_sample_with_oracle_5pt(
    n_point: int,
    max_scrambles: int,
    min_scrambles: int,
    max_scale: int,
    ...
    """Generate a sample with oracle trajectory using 5pt term-specific action space."""
    try:
        ...
        for i in range(len(reversed_states) - 1):
            ...
            action = compute_unscrambling_action_5pt(
...
def main():
    parser = argparse.ArgumentParser(description='Worker for 5pt oracle data generation')
    ...
    parser.add_argument('--max_brackets', type=int, default=20, help='Max brackets (20 for 5-point)')
    ...
    assert args.n_point == 5, "This script is specifically for 5-point amplitudes"
    ...
    while len(samples) < my_samples and attempts < max_attempts:
        attempts += 1
        sample = generate_sample_with_oracle_5pt(
            n_point=args.n_point,
    ...
    output_path = os.path.join(args.output_dir, f'oracle_5pt_{args.worker_id:04d}.pkl')
